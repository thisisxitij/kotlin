...
internal class BridgeLowering(val context: JvmBackendContext) : FileLoweringPass, IrElementTransformerVoid() {
    ...
    private fun IrSimpleFunction.copyParametersWithErasure(
        ...
        extensionReceiverParameter = from.extensionReceiverParameter?.copyWithTypeErasure(this, visibleTypeParameters, forceArgumentBoxing)
        valueParameters = if (substitutedParameterTypes != null) {
            from.valueParameters.zip(substitutedParameterTypes).map { (param, type) ->
                param.copyWithTypeErasure(this, visibleTypeParameters, forceArgumentBoxing, type)
            }
        } else {
            from.valueParameters.map { it.copyWithTypeErasure(this, visibleTypeParameters, forceArgumentBoxing) }
        }
    }
    ...
    private fun IrValueParameter.copyWithTypeErasure(
        ...
        type = (substitutedType?.eraseToScope(visibleTypeParameters) ?: type.eraseTypeParameters()).let { if (forceArgumentBoxing) it.makeNullable() else it },
